...
#include <string>
// Constructor
Monster ::Monster(const std ::string name, int frenzyOrder, bool hasFrenzy, bool isDefeated) : isDefeated(false), CurrentLocation(nullptr)
{
    this->Name = name;
    if (frenzyOrder >= 0)
...
bool Monster::GetDefeated() { return isDefeated; }
void Monster::MarkDefeated() {isDefeated = true;}
...
const std::shared_ptr<Location> &Monster ::GetLocation() const
{
    if (CurrentLocation)
        ...
        throw std::invalid_argument("No location Set for Monster " + Name);
}
...
const std ::shared_ptr<Location> Monster::FindNearestOpponent(Map &plan, std::shared_ptr<Location> monsterLocation, int monsterMoves)
{
    ...
    for (const auto &[name, locPtr] : plan.getLocations())
    {
        ...
        if (!villagers.empty())
        {
            ...
            else if (dist == minDist && !std::any_of(candidates.begin(), candidates.end(), [](const OpponentInfo &o)
                                                     { return o.isHero; }))
            {
                candidates.push_back({loc, dist, false});
            }
